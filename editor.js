import * as astro from 'https://unpkg.com/@astrojs/compiler@0.1.0-canary.1/browser/index.js?module';

let didInit = false;
const transform = async (value) => {
  if (!didInit) {
      await astro.initialize({
        wasmURL: 'https://unpkg.com/@astrojs/compiler@0.1.0-canary.1/astro.wasm'
      });
      didInit = true;
  }
  const res = await astro.transform(value);
  return res;
}

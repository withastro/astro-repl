...
const start = (port) => {
  ...
  if (_initialized) initEvent.emit('init');
        ...
        try {
          ...
          for (let data of models) {
            ...
            let result = await build({
              ...
              plugins: [ALIAS(), EXTERNAL(), ENTRY(filename), JSON_PLUGIN(), BARE(), HTTP(), CDN(), VIRTUAL_FS({ filename, transform }), WASM()],
              globalName: 'bundler',
            });
